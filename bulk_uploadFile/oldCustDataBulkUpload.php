    ...
    for ($i = 0; $i < $sheetCount; $i++) {
        ...
        foreach ($Reader as $Row) {
            ...
            if ($rowChange != 0) { // omitted 0 to avoid headers
                ...
                $data['line_name'] = $obj->getAreaGroup($con, $data['sub_area_id']) == $data['area_line'] ? $data['area_line'] : 'Invalid';
                ...
                $data['scheme_id'] = $obj->getSchemeId($con, $data['cus_reg_id']);

                $err_columns = $obj->handleError($data, $rowChange);
                if (empty($err_columns)) {

                    $obj->raiseRequest($con, $data, $userData);
