...
if($userid == 1){
    $query = 'SELECT * FROM in_verification where status = 0 and cus_status = 3 ';
}else{
    $query = "SELECT * FROM in_verification where status = 0 and cus_status = 3 and sub_area IN ($sub_area_list) ";//show only Approved Verification in Acknowledgement.
}
...
foreach ($result as $row) {
    ...
    if($cus_status == '3'){$sub_array[] = 'In Acknowledgement';}
    ...
    if($cus_status == '3') {
        $action .= "<a href='acknowledgement_creation&upd=$id&pge=1' class='customer_profile' value='$id' > Edit Documentation </a>";
    }
