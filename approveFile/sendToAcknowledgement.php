...
$insertACK = $con->query("INSERT INTO `in_acknowledgement`(`req_id`, `cus_id`, `cus_status`, `status`, `insert_login_id`) SELECT req_id,cus_id,cus_status,status,update_login_id from in_verification where req_id = '" . $req_id . "' ") or die('Error in in_acknowledgement');
$ack_id = $con->insert_id;
$qry = $con->query("UPDATE in_acknowledgement set inserted_user = '$userid', inserted_date = current_timestamp where `id` = '$ack_id' ");
