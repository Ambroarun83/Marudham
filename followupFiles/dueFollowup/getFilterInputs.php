...
//to get branches
$sql = $con->query("SELECT branch_id,branch_name FROM branch_creation where 1");
while($row = $sql->fetch_assoc()){
    $response['branch'][] = array('id'=>$row['branch_id'],'name'=>$row['branch_name']);
}

//to get Line
$sql = $con->query("SELECT  map_id,line_name,branch_id,area_id FROM area_line_mapping where 1 ");
while($row = $sql->fetch_assoc()){
    $response['line'][] = array('id'=>$row['map_id'],'name'=>$row['line_name'],'branch_id'=>$row['branch_id'],'area_id'=>$row['area_id']);
}

//to get Area
$sql = $con->query("SELECT area_id,area_name FROM area_list_creation where 1 ");
while($row = $sql->fetch_assoc()){
    $response['area'][]= array('id'=>$row['area_id'],'name'=>$row['area_name']);
}

//to get Sub Area
$sql = $con->query("SELECT sub_area_id,sub_area_name,area_id_ref FROM sub_area_list_creation where 1 ");
while($row = $sql->fetch_assoc()){
...
$sql = $con->query("SELECT loan_category_creation_id,loan_category_creation_name FROM loan_category_creation a
...
//to get Sub Categroy
$sql = $con->query("SELECT loan_category_id,sub_category_name,loan_category_name FROM loan_category where 1 ");
while($row = $sql->fetch_assoc()){
    $response['sub_cat'][] = array('id'=>$row['loan_category_id'],'name'=>$row['sub_category_name'],'loan_cat'=>$row['loan_category_name']);
}

//to get Agent
$sql = $con->query("SELECT ag_id,ag_name FROM agent_creation where 1 ");
while($row = $sql->fetch_assoc()){
