...
class promotionListClass
{
    ...
    function getdetails($con, $type)
    {
        ...
        if ($type == 'existing') {
            ...
            $sql = $con->query("SELECT req.*
                FROM request_creation req
                WHERE (req.cus_status >= 4 AND req.cus_status <= 9)
                AND (req.sub_area IN ( " . $this->sub_area_list . " ) or 
            ...
            while ($row = $sql->fetch_assoc()) {
                ...
                if ($sql1->num_rows > 0) {
                    ...
                    if ($arr['last_updated_date'] > $last_requested_date) {
    ...
    function getCustomerPromotionType($con,$cus_id,$cus_mob,$cus_name)
    {
        $arr = array();
        ...
        if($cus_id != ''){}//add  and (cs.cus_id = $cus_id) to query if cus id is not null due to we only checking cus id in table
        if ($sql->num_rows > 0) {
