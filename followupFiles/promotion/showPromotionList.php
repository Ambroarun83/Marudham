...
if (isset($_POST['search']) && $_POST['search'] != "") {
    $search = " and (cp.cus_id LIKE '%" . $_POST['search'] . "%' or cp.cus_name LIKE '%" . $_POST['search'] . "%' or cp.mobile1 LIKE '%" . $_POST['search'] . "%') ";
}
...
if ($type == 'existing') {
    ...
    $sql = $con->query("SELECT cs.cus_id, cs.consider_level, cs.updated_date 
        ...
        WHERE cs.cus_sts >= '20'  AND cp.area_confirm_subarea IN ($sub_area_list)  AND cs.closed_sts = 1  AND rc.cus_id IS NULL $search GROUP BY req.cus_id  $order  $limit ");
    ...
    $num_qry = $con->query("SELECT cs.cus_id, cs.consider_level, cs.updated_date 
        ...
        WHERE cs.cus_sts >= '20'  AND cp.area_confirm_subarea IN ($sub_area_list)  AND cs.closed_sts = 1  AND rc.cus_id IS NULL $search GROUP BY req.cus_id $order ");
...
foreach ($arr as $val) {
    ...
    if ($type == 'existing') {
        ...
        if ($type != 'existing') {
            ...
            if ($qry->num_rows > 0) {
                $sub_array[] = $qry->fetch_assoc()['prompt_remark'];
            } else {
                $sub_array[] = '';
            }
        }
