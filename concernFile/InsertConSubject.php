...
$crsNme='';
$crsStatus='';
$selectCategory=$con->query("SELECT * FROM concern_subject WHERE concern_subject = '".$com_sub_add."' ");
...
if($crsNme != '' && $crsStatus == 0){
	...
	$updateCategory=$con->query("UPDATE concern_subject SET status=0 WHERE concern_subject='".$com_sub_add."' ");
	$message="Concern Subject Added Succesfully";
}
else{
	if($con_sub_id>0){
		$updateCategory=$con->query("UPDATE concern_subject SET concern_subject='".$com_sub_add."' WHERE con_sub_id='".$con_sub_id."' ");
		if($updateCategory == true){
		    $message="Concern Subject Updated Succesfully";
	    }
    }
	else{
	    $insertCategory=$con->query("INSERT INTO concern_subject(concern_subject) VALUES('".strip_tags($com_sub_add)."')");
