...
// $cus_id=123456789101;
$req_arr = array();
$qry=$con->query("SELECT req_id FROM in_issue where cus_id = $cus_id  "); //and (cus_status = 20)
...
function calculateOthers($loan_arr,$response,$con,$req_id){
    ...
    if($loan_arr['due_method_calc'] == 'Monthly' || $loan_arr['due_method_scheme'] == '1'){
        ...
        $qry = $con->query("SELECT SUM(c.due_amt_track) as due_amt_track,SUM(c.princ_amt_track) as princ_amt_track, SUM(pc.penalty) as penalty, SUM(pc.paid_amnt) as paid_amntpc,SUM(pc.waiver_amnt) as waiver_amntpc,
        ...
        if($loan_arr['loan_type']=='interest'){
            $due_amt_track = $row['princ_amt_track'];
        }
        ...
        if($row['waiver_amntpc'] == '' or $row['waiver_amntpc'] == null){$row['waiver_amntpc'] = 0;}
