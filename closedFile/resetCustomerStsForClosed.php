...
// $cus_id=123456789101;
$req_arr = array();
$qry = $con->query("SELECT req_id FROM in_issue where cus_id = $cus_id ORDER BY CAST(req_id AS UNSIGNED) ASC "); // and (cus_status < 21 and cus_status >= 14)"); //and (cus_status = 20)
...
function calculateOthers($loan_arr, $response, $con, $req_id)
{
    ...
    if ($loan_arr['due_method_calc'] == 'Monthly' || $loan_arr['due_method_scheme'] == '1') {
        if ($loan_arr['loan_type'] != 'interest') {
            ...
            $interest_details = calculateInterestLoan($con, $loan_arr, $response, $req_id);
...
function calculateInterestLoan($con, $loan_arr, $response, $req_id)
{

    $due_start_from = $loan_arr['loan_date'];
    $maturity_month = $loan_arr['maturity_month'];

    $checkcollection = $con->query("SELECT SUM(`int_amt_track`) as totalPaidAmt FROM `collection` WHERE `req_id` = '$req_id'"); // To Find total paid amount till Now.
    ...
    if ($count > 0) {
        $interest_paid = getPaidInterest($con, $req_id);

        $res['payable'] = payableCalculation($con, $loan_arr, $response, $req_id) - $interest_paid;
        $res['till_date_int'] = getTillDateInterest($loan_arr, $response, $con, 'curmonth', $req_id) - $interest_paid;
        $res['pending'] = pendingCalculation($con, $loan_arr, $response, $req_id) - $interest_paid;
        ...
        $res['penalty'] = getPenaltyCharges($con, $req_id);
    } else {
        //in this calculate till date interest when month are not crossed for due starting month
        $res['till_date_int'] = getTillDateInterest($loan_arr, $response, $con, 'forstartmonth', $req_id);
        $res['pending'] = 0;
        $res['payable'] = 0;
        $res['penalty'] = 0;
    }
...
function dueAmtCalculation($con, $start_date, $end_date, $due_amt, $loan_arr, $status, $req_id)
{
    // var_dump($start_date);
    ...
    $result = 0;
    $qry = $con->query("SELECT princ_amt_track FROM `collection` WHERE req_id = '" . $req_id . "' and princ_amt_track != '' ORDER BY coll_date ASC ");
    if ($qry->num_rows > 0) {
        ...
        while ($start->format('m') <= $end->format('m')) {
            ...
            $qry = $con->query("SELECT princ_amt_track as princ,bal_amt, coll_date FROM `collection` WHERE req_id = '" . $req_id . "' and princ_amt_track != '' and month(coll_date) = month('" . $start->format('Y-m-d') . "') and year(coll_date) = year('" . $start->format('Y-m-d') . "') ORDER BY coll_date ASC ");
            if ($qry->num_rows > 0) {
                ...
                $qry = $con->query("SELECT princ_amt_track as princ,bal_amt, coll_date FROM `collection` WHERE req_id = '" . $req_id . "' and princ_amt_track != '' and month(coll_date) < month('" . $start->format('Y-m-d') . "') and year(coll_date) <= year('" . $start->format('Y-m-d') . "') ORDER BY coll_date ASC LIMIT 1");
                if ($qry->num_rows > 0) {
                    $row = $qry->fetch_assoc();
                    $princ = $row['princ'];
                    $bal_amt = $row['bal_amt'];
                    ...
                    $qry = $con->query("SELECT principal_amt_cal from acknowlegement_loan_calculation where req_id = '" . $req_id . "' ");
                    $row = $qry->fetch_assoc();
            ...
            if ($status == 'pending') { //raising penalty if loops for looping month
                ...
                $paid_interest = getPaidInterest($con, $req_id);
                if ($paid_interest > 0) {
...
function payableCalculation($con, $loan_arr, $response, $req_id)
{
    ...
    while ($st_date->format('m') <= $last_month->format('m')) {
        ...
        $result += dueAmtCalculation($con, $start, $end_date, $response['due_amt'], $loan_arr, 'payable', $req_id);
...
function pendingCalculation($con, $loan_arr, $response, $req_id)
{
    $pending = getTillDateInterest($loan_arr, $response, $con, 'pendingmonth', $req_id);
    return $pending;
}
function getTillDateInterest($loan_arr, $response, $con, $data, $req_id)
{
    ...
    if ($data == 'forstartmonth') {
        ...
        if ($loan_arr['loan_type'] == 'interest') {
            ...
            $result = dueAmtCalculation($con, $issued_date, $cur_date, $response['due_amt'], $loan_arr, '', $req_id);
    ...
    if ($data == 'curmonth') {
        ...
        $result = dueAmtCalculation($con, $issued_date, $cur_date, $response['due_amt'], $loan_arr, 'TDI', $req_id);
        return $result;
    }
    if ($data == 'pendingmonth') {
        ...
        if ($issued_date->format('m') <= $cur_date->format('m')) {
            $result = dueAmtCalculation($con, $issued_date, $cur_date, $response['due_amt'], $loan_arr, 'pending', $req_id);
        }
        return $result;
    }
...
function getPaidInterest($con, $req_id)
{
    $qry = $con->query("SELECT SUM(int_amt_track) as int_paid FROM `collection` WHERE req_id = '$req_id' and (int_amt_track != '' and int_amt_track IS NOT NULL) ");
    $int_paid = $qry->fetch_assoc()['int_paid'];
    return intVal($int_paid);
}
function getPenaltyCharges($con, $req_id)
{
