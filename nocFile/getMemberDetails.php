...
if($noc_member == 2){
    $qry = $con->query("SELECT cp.guarentor_name,fam.famname,fp.ansi_template FROM acknowlegement_customer_profile cp JOIN verification_family_info fam ON fam.id = cp.guarentor_name
    JOIN fingerprints fp ON fam.relation_aadhar = fp.adhar_num WHERE cp.req_id='$req_id' ");
    $row = $qry->fetch_assoc();
